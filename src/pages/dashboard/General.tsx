...
export default function GeneralDashboard() {
    ...
    useEffect(() => {
        getGeneralKPI(filters.period, filters.platform).then(setData);
    }, [filters]);
    ...
    return (
        <div className="p-8 pb-32">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                {/* 1. Active Users & Sessions Trend */}
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            <h3 className="text-lg font-bold text-gray-800">유저당 평균 세션 수</h3>
                            <p className="text-xs text-gray-400 mt-1">방문 빈도 추이 및 세션 집중도</p>
                        </div>
                        <div className="text-right">
                            <span className="text-[10px] font-bold text-gray-400 block uppercase">평균 수치</span>
                    ...
                    <div className="h-[300px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <ComposedChart data={data.charts}>
                                ...
                                />
                                <Bar yAxisId="left" dataKey="sessions" fill="#e2e8f0" radius={[4, 4, 0, 0]} name="총 세션" />
                                <Line yAxisId="right" type="monotone" dataKey="dau" stroke="#00A980" strokeWidth={3} dot={{ r: 4, fill: '#00A980', strokeWidth: 2, stroke: '#fff' }} name="일일 유저" />
                            </ComposedChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            <h3 className="text-lg font-bold text-gray-800">평균 체류 시간</h3>
                            <p className="text-xs text-gray-400 mt-1">목표 체류 시간(3분) 대비 달성 현황</p>
                        </div>
                        <div className="text-right">
                            <span className="text-[10px] font-bold text-gray-400 block uppercase">현재 평균</span>
                    ...
                    <div className="h-[300px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={data.charts}>
                                ...
                                <Tooltip />
                                <ReferenceLine y={180} label={{ value: '목표 (3분)', position: 'right', fill: '#ef4444', fontSize: 10, fontWeight: 'bold' }} stroke="#ef4444" strokeDasharray="5 5" />
                                <Area type="monotone" dataKey="sessionDuration" stroke="#3b82f6" fillOpacity={1} fill="url(#colorDur)" strokeWidth={3} name="체류시간 (초)" />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-sm border border-gray-100 h-fit">
                    <div className="flex justify-between items-start mb-6">
                        <div>
                            <h3 className="text-lg font-bold text-gray-800">앱 안정성</h3>
                        ...
                        <div className={`px-2 py-1 rounded text-[10px] font-black uppercase tracking-widest ${parseFloat(data.metrics?.crashFreeUsers) >= 99 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                            ...
                            {parseFloat(data.metrics?.crashFreeUsers) >= 99 ? 'Normal' : 'Warning'}
                        </div>
                    </div>
